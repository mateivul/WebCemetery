class StatsCalculator {
    ...
    async getKilledCounts() {
        ...
        // const monthCount = allStats.reduce((sum, s) => sum + s.count, 0);
        // const totalCount = await this.storage.getTotalCount();

        const allTombstones = await this.storage.getAllTombstones();

        const todayCount = allTombstones.filter((t) => this.getDateString(new Date(t.killedAt)) === todayStr).length;
