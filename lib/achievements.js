class AchievementManager {
    constructor(store) {
        ...
        this._tombstonesCacheTime = 0;
        this._cacheValidityMs = 30000;
    }

    async getCachedTombstones() {
        const now = Date.now();
        if (this._tombstonesCache && now - this._tombstonesCacheTime < this._cacheValidityMs)
            return this._tombstonesCache;

        try {
            this._tombstonesCache = await this.storage.getAllTombstones();
            this._tombstonesCacheTime = now;
            return this._tombstonesCache;
        } catch (error) {
    ...
    defineAchievements() {
        return {
            ...
            resurrector: {
                ...
                description: "Resurrect 10 dead tabs",
                icon: "🧟",
                rarity: "uncommon",
    ...
    async meetsRequirement(requirement, stats, tombstoneData) {
        switch (requirement.type) {
            ...
            case "quickKills":
                return tombstoneData ? tombstoneData.timeAlive <= requirement.value : false;
    ...
    async getCurrentStreak() {
        if (!this.storage || typeof this.storage.getAllStats !== "function") return 0;
